...
**Phase 5 is shipped.** `useItemsManager`/`ItemRow`/`ArchivedItemRow`/`ItemsContext` are the only implementations in production; every pre-merge job/claim duplicate pair (`useDiagnosticsManager` hook + `useClaimMaterialsManager`, `SparePartsRow`/`ClaimSparePartsRow`, `ArchivedSparePartsRow`/inlined claim archived row) has been deleted, with full historical test coverage preserved via 1:1-verified ports. `useDiagnosticsManager.ts` remains as a file, holding only the still-live job-diagnostic-tab helpers (`computeIsChargeable`, `hasWarrantyOrProServiceItems`, `getChargeablePendingInfo`, `getBoschInternalPending`) and the `materialsDerivation.ts` re-export block for backward compatibility.
