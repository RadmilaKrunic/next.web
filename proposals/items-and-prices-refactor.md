...
## 15. Phase 5: unifying Job/Claim item rows (in progress — steps 1–9 of 10 done)
...
- **Step 10: in progress** — full 1:1 historical test-parity porting, then deletion of every old implementation, per the plan's own safety gate. Done so far:
  ...
  - Deleted (old source + test, 1:1 coverage confirmed): `useClaimMaterialsManager.ts`/`.test.ts`, `ArchivedSparePartsRow.tsx`/`.test.ts` (already fully covered by `ArchivedItemRow.test.tsx`, confirmed case-by-case).
  - In progress: full 1:1 port of `SparePartsRow.test.tsx` (2037 lines)/`ClaimSparePartsRow.test.tsx` (691 lines) into `ItemRow.test.tsx`, before `SparePartsRow.tsx`/`ClaimSparePartsRow.tsx` and their test files can be deleted.
