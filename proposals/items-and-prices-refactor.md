...
| **4** | Replace Effect 3/3b's incremental Area-cloning reconciliation with full recomputation from `materials` on every change; simplify `onDeleteRow`/`onRestoreRow` accordingly (§7) | Medium (touches `useDiagnosticsManager.ts`; field naming, `mapValuesToAPI`, `GenericField.tsx` untouched) — lower than the originally-planned store extraction | Medium | Yes — pure FE reconciliation-strategy fix, no backend dependency at all, can ship before Phase 3 |
